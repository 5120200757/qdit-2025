...
# 计算 FID、sFID、Precision、Recall、Inception Score 指标
# 图像形状为(batches, image_size, image_size, channels)

# CUDA_VISIBLE_DEVICES=6  python evaluator.py evaluation/imagenet256.npz output/256_48_50_ref/inference/sample_batch.npz
...
class Evaluator:
    ...
    def compute_activations(self, batches: Iterable[np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 返回所有批次的池化特征、空间特征 合并成一个大的numpy数组
        return (
    ...
    def read_statistics(
        self, npz_path: str, activations: Tuple[np.ndarray, np.ndarray]  # 那个activations只跟['arr_0']的数据有关
...
def open_npz_array(path: str, arr_name: str) -> NpzArrayReader:
    # arr_f 是指 ref/sample_batch['arr_0']
    with _open_npy_file(path, arr_name) as arr_f:
        ...
        if version == (1, 0):
            ...
            return
        # 获取数组头信息: 形状、布尔值( 数组是否为Fortran顺序 )、数据类型
        shape, fortran, dtype = header
        if fortran or dtype.hasobject:
            # 数组较小，整个数组加载到内存，访问数据效率高
            yield MemoryNpzArrayReader.load(path, arr_name)
        else:
            # 数组较大，按需从文件中读取数据，内存效率搞
