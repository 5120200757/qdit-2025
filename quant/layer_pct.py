...
def layer_tunepct(
        model: QuantModel,
                  layer: QuantModule,
                  cali_data: torch.Tensor,          # 保留但不使用
                  iters: int = 400,
                  lr: float = 1e-3,
                  clip_reg: float = 0.0,            # >0 时加入裁剪比例惩罚
                  min_pct: float = 0.9,             # 约束 pct 下界
                  max_pct: float = 0.99999,         # 约束 pct 上界
                  log_interval: int = 20,
                  outpath: str = None):
    ...
    if layer.weight_quantizer.pat:
        if not isinstance(layer.weight_quantizer.pct, torch.Tensor):
            layer.weight_quantizer.pct = nn.Parameter(torch.tensor(float(layer.weight_quantizer.pct), device=layer.weight.device))
        elif not isinstance(layer.weight_quantizer.pct, nn.Parameter):
            layer.weight_quantizer.pct = nn.Parameter(layer.weight_quantizer.pct.to(layer.weight.device))
        
        opt_params_w = [layer.weight_quantizer.pct]
        optimizer_w = torch.optim.Adam(opt_params_w, lr=lr)

    if layer.act_quantizer.pat:
        if not isinstance(layer.act_quantizer.pct, torch.Tensor):
            layer.act_quantizer.pct = nn.Parameter(torch.tensor(float(layer.act_quantizer.pct), device=layer.weight.device))
        elif not isinstance(layer.act_quantizer.pct, nn.Parameter):
            layer.act_quantizer.pct = nn.Parameter(layer.act_quantizer.pct.to(layer.weight.device))
        opt_params_a = [layer.act_quantizer.pct]
    ...
    # torch.set_grad_enabled() vs. layer.train()
    # 前者：用于控制整个上下文的梯度计算 ( 是否构建计算图.. )
    # 后者：用于设置特定模块的训练模式 ( 训练/评估 )
